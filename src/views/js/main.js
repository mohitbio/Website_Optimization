...
var selectRandomMeat = function() {
    return randomMeat = pizzaIngredients.meats[Math.floor((Math.random() * pizzaIngredients.meats.length))];
};

var selectRandomNonMeat = function() {
    return randomNonMeat = pizzaIngredients.nonMeats[Math.floor((Math.random() * pizzaIngredients.nonMeats.length))];
};

var selectRandomCheese = function() {
    return randomCheese = pizzaIngredients.cheeses[Math.floor((Math.random() * pizzaIngredients.cheeses.length))];
};

var selectRandomSauce = function() {
    return randomSauce = pizzaIngredients.sauces[Math.floor((Math.random() * pizzaIngredients.sauces.length))];
};

var selectRandomCrust = function() {
    return randomCrust = pizzaIngredients.crusts[Math.floor((Math.random() * pizzaIngredients.crusts.length))];
};
...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        /*After testing the 'querySelectorAll' from the original code, it was slowing down this function becuase
          its searching the entire DOM to find the elements that it was looking for. In order to fix that, the 'querySelectorAll'
          needs to be called outside since it will call all of them for you once.
         */
...
function updatePositions() {
    ...
    var scrollup = document.body.scrollTop,
        scrolllength = items.length,
        updatepos = [],
        fitscroll = scrollup / 1250;

    /*This will calculate the new positions where it will loop at least 5 times (or the maximum scrolllength)
    ...
    for (var i = 0; i < scrolllength; i++) {
        var item = items[i];
        item.style.left = item.basicLeft + updatepos[i % 5] + 'px';
    }
...
//Adding items as global for storing other elements outside.
var items = [];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    * from querySelectorAll from above, it will also reduce the amount of calls.*/
    ...
    window.addEventListener('scroll', updatePositions); //This will update the positions when being scrolled.
    updatePositions();
});